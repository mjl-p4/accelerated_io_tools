...
$IQ "aio_save(build(<x:int64>[i=-10:10:0:21], i), '$F', atts_only:true, format:'arrow', atts_only:false)" 2>&1     \
    |  sed --expression='s/ line: [0-9]\+//g'                                                                   \
    |  grep --invert-match "Failed query id:" >> $TEST_OUT                                                      \
    || echo "expected exception"
...
echo -e "\nIV. atts_only"
echo "1. Single chunk (atts_only:true)"
iq "aio_save(apply(build(<x:int64>[i=-10:10:0:21], i), y, iif(i%2=0, i, int64(null))), '$F', format:'arrow', atts_only:true)"
python -c "import pyarrow; print(pyarrow.open_stream('$F').read_all().to_pandas())" \
    >> $TEST_OUT

echo "2. Single chunk (atts_only:false)"
iq "aio_save(apply(build(<x:int64>[i=-10:10:0:21], i), y, iif(i%2=0, i, int64(null))), '$F', format:'arrow', atts_only:false)"
python -c "import pyarrow; print(pyarrow.open_stream('$F').read_all().to_pandas())" \
    >> $TEST_OUT

echo "3. Multiple chunks (atts_only:true)"
iq "aio_save(apply(build(<x:int64>[i=1:100:0:20], i), y, iif(i%2=0, i, int64(null))), '$F', format:'arrow', atts_only:true)"
python -c "import pyarrow; print(pyarrow.open_stream('$F').read_all().to_pandas().sort_values('x').to_string(index=False))" \
    >> $TEST_OUT

echo "4. Multiple chunks (atts_only=false)"
iq "aio_save(apply(build(<x:int64>[i=1:100:0:20], i), y, iif(i%2=0, i, int64(null))), '$F', format:'arrow', atts_only:false)"
python -c "import pyarrow; print(pyarrow.open_stream('$F').read_all().to_pandas().sort_values('x').to_string(index=False))" \
    >> $TEST_OUT

echo "5. datetime (atts_only=false)"
iq "aio_save(apply(build(<x:datetime not null>[i=1:20:0:4], i), y, iif(i%2=0, datetime(i), datetime(null))), '$F', format:'arrow', atts_only:false)"
python -c "import pyarrow; print(pyarrow.open_stream('$F').read_all().to_pandas().sort_values('x').to_string(index=False))" \
    >> $TEST_OUT

echo "6. double (atts_only=false)"
iq "aio_save(apply(build(<x:double not null>[i=1:20:0:4], i), y, iif(i%2=0, double(i), double(null))), '$F', format:'arrow', atts_only:false)"
python -c "import pyarrow; print(pyarrow.open_stream('$F').read_all().to_pandas().sort_values('x').to_string(index=False))" \
    >> $TEST_OUT

echo "7. float (atts_only=false)"
iq "aio_save(apply(build(<x:float not null>[i=1:20:0:4], i), y, iif(i%2=0, float(i), float(null))), '$F', format:'arrow', atts_only:false)"
python -c "import pyarrow; print(pyarrow.open_stream('$F').read_all().to_pandas().sort_values('x').to_string(index=False))" \
    >> $TEST_OUT

echo "8. int8 (atts_only=false)"
iq "aio_save(apply(build(<x:int8 not null>[i=1:20:0:4], i), y, iif(i%2=0, int8(i), int8(null))), '$F', format:'arrow', atts_only:false)"
python -c "import pyarrow; print(pyarrow.open_stream('$F').read_all().to_pandas().sort_values('x').to_string(index=False))" \
    >> $TEST_OUT

echo "9. string (atts_only=false)"
iq "aio_save(apply(build(<x:string not null>[i=1:20:0:4], i), y, iif(i%2=0, string(i), string(null))), '$F', format:'arrow', atts_only:false)"
python -c "import pyarrow; print(pyarrow.open_stream('$F').read_all().to_pandas().sort_values('x').to_string(index=False))" \
    >> $TEST_OUT

echo "10. uint32 (atts_only=false)"
iq "aio_save(apply(build(<x:uint32 not null>[i=1:20:0:4], i), y, iif(i%2=0, uint32(i), uint32(null))), '$F', format:'arrow', atts_only:false)"
python -c "import pyarrow; print(pyarrow.open_stream('$F').read_all().to_pandas().sort_values('x').to_string(index=False))" \
    >> $TEST_OUT

echo "11. Buffer size"
iq "aio_save(filter(build(<x:int64>[i=1:100:0:50], i), x % 2 = 0), '$F', format:'arrow', atts_only:false, buffer_size:256)"
python -c "import pyarrow; print(pyarrow.open_stream('$F').read_all().to_pandas().sort_values('x').to_string(index=False))" \
    >> $TEST_OUT
